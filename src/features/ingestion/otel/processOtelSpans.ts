...
// ── Attribute extraction (simplified from langfuse OtelIngestionProcessor) ──
...
function extractObservationType(attrs: Record<string, unknown>): ObservationType {
  // Explicit type from langfuse SDK
...
function extractUsageDetails(attrs: Record<string, unknown>): {
  ...
  // Langfuse SDK sends usage as JSON string
  ...
  // OTel semantic conventions fallback
  const input =
...
function extractCompletionStartTime(attrs: Record<string, unknown>): Date | null {
  ...
  try {
    let value = raw;
    // Handle double-stringified timestamps
...
async function processSpan(
  ...
  // Upsert trace for root spans or spans with trace-level attributes
  if (isRootSpan || hasTraceUpdates(attrs)) {
    ...
    // Non-root span without trace updates — just ensure trace exists
